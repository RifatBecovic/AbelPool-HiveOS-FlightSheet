#!/bin/bash
#

action=${1-}
address=${2-}
homepath="/root/pool_dir"

function usage() {
  echo "==================== Help Usage ====================" 
  echo "Deployment Management Script"
  echo "Usage: "
  echo "  ./allinone_ctl.sh [COMMAND] [ADDRESS:PASS]"
  echo "  ./allinone_ctl.sh --help"
  echo
  echo "Example:"
  echo "  ./allinone_ctl.sh start account:pass"
  echo
  echo "Management Commands: "
  echo "  start   addrxx:passxx  ( Auto Detect Binary Update and Start Binary Process. )"
  echo "  update  addrxx:passxx  ( ForceUpdate Binary Process. )"
  echo "  restart addrxx:passxx  ( Restart     Binary Process. )"
  echo "  stop                   ( Stop        Binary Process. )"
  echo
  echo
}

function upgrade_glibc() {
    echo "INFO: Now auto update glibc ......"
    sysversion=`cat /etc/os-release | grep "PRETTY_NAME=" | awk -F= '{print $2}' | grep -i 'Ubuntu 20.' > /dev/null  && echo 'true' || echo 'false'`
    if [ $sysversion = 'true' ]; then
        echo 'deb http://th.archive.ubuntu.com/ubuntu jammy main' >> /etc/apt/sources.list
        sudo apt update
        DEBIAN_FRONTEND=noninteractive apt-get -y install libc6
    else
        echo "Error: only support ubuntu 20.x upgrade."
        echo "Upgrade HiveOS ubuntu 20.x command: hive-replace -y https://download.hiveos.farm/history/hiveos-0.6-224-beta@230911.img.xz"
        exit 1
    fi
}

function checkfile() {
    local check_file="$homepath/$1"
    if [ -s $check_file ]; then
	#检测 miner
        if echo $check_file | grep abelminer >/dev/null ;then
	    if md5sum $check_file | grep '67aa92ef0c938e83b5ae60986437fc3e' >/dev/null ; then
		return 0
	    else
		return 1
	    fi
	#检测 heartbeat
	elif echo $check_file | grep machine-heartbeat >/dev/null ;then
            if md5sum $check_file | grep 'b6c7f20b812c61706a10021be6209b7d' >/dev/null ;then
                return 0
            else
                return 1
            fi
        #check run_miner.sh
	elif echo $check_file | grep run_miner.sh >/dev/null ;then
            if md5sum $check_file | grep '1816636c88f1697406f9f70e3883f898' >/dev/null ;then
                return 0
            else
                return 1
            fi
        #check run_heartbeat.sh
	elif echo $check_file | grep run_heartbeat.sh >/dev/null ;then
            return 0
	else
	    return 1
	fi
    else
	return 1
    fi
}

function download_miner() {
  echo "INFO: download miner soft ......"
  rm -rf $homepath/abelminer*
  mkdir $homepath 2>/dev/null
  cd $homepath
  wget -q -O $homepath/pool.tar.gz https://download.abelpool.io/cmd_global/pool.tar.gz
  echo "INFO: unzip ......"
  tar xzf pool.tar.gz 
}

function download_hearbeat() {
  curlret=`curl https://api.abelpool.io/api/v1/ping 2>/dev/null`
  if [ $curlret == "pong" ]; then
      echo "INFO: connect api.abelpoo.io   [ Pass ]"
  else
      echo "INFO: connect api.abelpoo.io   [ NoPass ]"
      if grep 'api.abelpool.io' /etc/hosts >/dev/null ; then
          echo "INFO: connect api.abelpoo.io   [ NoFixed ]"
      else
          echo '104.21.0.152   api.abelpool.io'  >> /etc/hosts
          echo "INFO: connect api.abelpoo.io   [ Fixed ]"
      fi
  fi
  rm -rf $homepath/machine-heartbeat*
  mkdir $homepath 2>/dev/null
  cd $homepath
  echo "INFO: download machine-heartbeat ......"
  wget -q -O $homepath/heartbeat.tar.gz https://download.abelpool.io/cmd_global/heartbeat.tar.gz
  echo "INFO: unzip machine-heartbeat ......"
  tar xzf heartbeat.tar.gz
}

function ntp_timezone() {
  echo "INFO: Now set timezone ..."
  #校准时区 UTC 
  \cp /usr/share/zoneinfo/UTC /etc/localtime 2>/dev/null
  timedatectl set-timezone UTC  2>/dev/null

  if command -v ntpdate >/dev/null ;then
      echo "INFO: Find ntpdate. "
  else
      echo "INFO: Can't Find ntpdate, Install ... "
      apt update -y  >/dev/null 2>&1
      apt install ntpdate -y >/dev/null 2>&1
  fi
  #校准时间
  ntpdate -u 0.rhel.pool.ntp.org 1.rhel.pool.ntp.org 2.rhel.pool.ntp.org 3.rhel.pool.ntp.org time.windows.com time1.google.com time2.google.com time3.google.com ntp.aliyun.com ntp1.aliyun.com ntp2.aliyun.com ntp3.aliyun.com ntp.ubuntu.com
}

function check_cert() {
    filename=$1
    sslfile="$homepath/abelminer-linux-amd64-v2.0.2/poolcerts/$filename"

    if [ -s "$sslfile" ]; then
	echo "INFO: check $filename [ Pass ] "
    else
	echo "INFO: check $filename [ NoPass ] "
        echo "-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----" > $sslfile
	echo "INFO: check $filename [ Fixed ] "
    fi
}

function check_gpu_power() {
    for runtimes in `seq 60`; do
        if ps aux | grep abelminer | grep abelpool.io | grep stratum >/dev/null 2>&1 ; then
            powerCount=`tail -n 10 /root/miner.log | sed -r "s/\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]//g" | grep -c -E 'Mh - cu|Gh - cu|Kh - cu|Mh - cl'`
            if [ $powerCount -gt 3  ] ; then
                echo "INFO: Find GPU Power, AbelMiner GPU Running  [ OK ]"
                tail -n 20 /root/miner.log
                return
            else
                echo "INFO: Waiting GPU power, sleep times -> $runtimes"
                sleep 15
            fi
        else
            echo "INFO: Can't Find Porcess , sleep wait times -> $runtimes"
            sleep 3
        fi
    done
    echo 
    echo "INFO: Can't Find GPU power, AbelMiner start Failed  [ Error ]"
    echo "INFO: Please check /root/miner.log manually!"
    echo "check command: tail -n 20 /root/miner.log"
    tail -n 20 /root/miner.log
}

function check_abelminer_run() {
    for runtimes in `seq 60`;
    do
        if ps aux | grep abelminer | grep abelpool.io | grep stratum >/dev/null 2>&1 ; then
            echo "INFO: AbelMiner Started.  [ OK ]"
	    return
        else
            sleep 1
        fi
    done
    echo "INFO: AbelMiner Start Failed.  [ Err]"
}

function check_machine_heartbeat_run() {
    for runtimes in `seq 60`;
    do
        if ps aux | grep machine-heartbeat | grep -v grep >/dev/null 2>&1 ; then
            echo "INFO: machine-heartbeat Started.  [ OK ]"
	    tail /root/heartbeat.log
	    return
        else
            sleep 1
        fi
    done
    echo "INFO: machine-heartbeat Start Failed.  [ Err]"
    tail /root/heartbeat.log
}

function start() {
  ### check Parm 
  if [ ! -n "$1" ];then
      echo
      echo "Error: start need Parm -> accountxx:passxx"
      echo
      usage
      exit 1
  fi
 
  ### 校准时间
  ntp_timezone

  ### glic block
  if strings -v >/dev/null 2>&1; then
      glibc_version=`strings /lib/x86_64-linux-gnu/libc.so.6 | grep GLIBC_2.3[2-5] > /dev/null  && echo 'true' || echo 'false'`
      if [ $glibc_version = 'true' ]; then
          echo "INFO: check glibc >= GLIBC_2.32    [ Pass ]"
      else
          echo "INFO: check glibc < GLIBC_2.32    [ NoPass ]"
          upgrade_glibc
      fi
  else
      echo "Warning: strings command not found, skip glibc check ."
  fi

  ### miner block
  if ! checkfile "abelminer-linux-amd64-v2.0.2/abelminer"; then
      echo "INFO: check localfile -> abelminer  [ Nopass ]"
      download_miner
  else
      echo "INFO: check localfile -> abelminer  [ Pass ]"
  fi

  if ! checkfile "run_miner.sh"; then
      echo "INFO: check localfile -> run_miner.sh  [ Nopass ]"
      wget -q -O $homepath/run_miner.sh https://download.abelpool.io/cmd_global/pool_dir/run_miner.sh
  else
      echo "INFO: check localfile -> run_miner.sh  [ Pass ]"
  fi

  #check 证书
  check_cert "global-service.abelpool.io.cert"
  check_cert "asia-service.abelpool.io.cert"
  echo "INFO: run abelminer ......"
  cd $homepath
  chmod +x run_miner.sh
  nohup ./run_miner.sh "$1" &> /dev/null &
  echo "INFO: check abelminer process ......"
  check_abelminer_run
  echo

  ### bearbeat block
  if ! checkfile "machine-heartbeat"; then
      echo "INFO: check localfile -> machine-heartbeat  [ Nopass ]"
      download_hearbeat
  else
      echo "INFO: check localfile -> machine-heartbeat  [ Pass ]"
  fi

  if ! checkfile "run_heartbeat.sh"; then
      echo "INFO: check localfile -> run_heartbeat.sh  [ Nopass ]"
      download_hearbeat
  else
      echo "INFO: check localfile -> run_heartbeat.sh  [ Pass ]"
  fi

  echo "INFO: run machine-heartbeat ......"
  cd $homepath
  chmod +x machine-heartbeat
  chmod +x run_heartbeat.sh
  nohup ./run_heartbeat.sh &> /dev/null &
  echo "INFO: check machine-heartbeat process ......"
  check_machine_heartbeat_run
  echo
  
  echo "INFO: Check GPU Power ......"
  check_gpu_power
}

function stop() {
    echo "INFO: kill process ......"
    for i in `seq 1 5`
    do
        ps aux | grep miner | grep -v grep  | awk '{print $2}' | xargs kill -9 >/dev/null 2>&1
        ps aux | grep heartbeat | grep -v grep  | awk '{print $2}' | xargs kill -9  >/dev/null 2>&1
        sleep 1
    done
    echo "INFO: kill process OK."
}

function main() {

  case "${action}" in
  start)
    echo "INFO: action-> start"
    stop
    start $2
    ;;
  restart)
    stop
    start $2
    ;;
  stop)
    echo "INFO: action-> stop"
    stop
    ;;
  update)
    rm -rf $homepath
    stop
    start $2
    ;;
  help)
    usage
    ;;
  --help)
    usage
    ;;
  -h)
    usage
    ;;
  *)
    echo 
    echo "Error: No such command: ${action}"
    echo 
    usage
    ;;
  esac
}

main "$@"
